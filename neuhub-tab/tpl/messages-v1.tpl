    ...
    <nav>
        ...
        {{else}}
            <ul class="pagination justify-content-center">
                ...
                {{if $max_page>1}}
                    {{for $i=1 to $max_page}}
                        <li class="d-flex justify-content-center align-items-centerd-flex justify-content-center align-items-center page-item {{if $i == $page}} active {{/if}}"><a class="page-link" href="{{$base_url}}/messages?type={{$type}}&page={{$i}}">{{$i}}</a></li>
                    {{/for}}
                {{/if}}
